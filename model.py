...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.key_dim == 64 and self.query_dim == 64:
            ...
            # key, query, value projections for all heads, but in a batch
            self.c_attn = nn.Linear(config.n_embd, self.query_dim * config.n_head + self.key_dim * config.n_head + config.n_embd, bias=config.bias)
            # output projection
            self.c_proj = nn.Linear(config.n_embd, config.n_embd, bias=config.bias)
        # regularization
    ...
    def forward(self, x):
        ...
        if self.key_dim == 64 and self.query_dim == 64:
            ...
            # calculate query, key, values for all heads in batch and move head forward to be the batch dim
            q, k, v  = self.c_attn(x).split([self.query_dim * self.n_head, self.key_dim * self.n_head, self.n_embd], dim=2)
            k = k.view(B, T, self.n_head, self.key_dim).transpose(1, 2) # (B, nh, T, hs)
            q = q.view(B, T, self.n_head, self.query_dim).transpose(1, 2) # (B, nh, T, hs)
            v = v.view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
